...
import { LogOut, User, Package, MapPin, CreditCard, ChevronRight } from 'lucide-react';
...
export default function ProfilePage() {
  ...
  if (!user) {
    return (
      <div className="flex flex-col items-center justify-center min-h-[60vh]">
        <p className="mb-4 text-gray-500">Вы не авторизованы</p>
        <button 
          onClick={() => router.push('/login')}
          className="bg-[#1565C0] text-white px-6 py-2 rounded-xl"
        >
          Войти
        </button>
      </div>
  ...
  // Массив меню для удобного рендера
  const menuItems = [
    { 
      id: 'data', 
      label: 'Мои данные', 
    ...
    <div className="w-[1340px] mx-auto py-[20px] flex">
      ...
      <aside className="">
        <Link href="/" className="flex gap-[10px] items-center my-[20px]">
          ...
          <p className="text-[16px]">Вернуться</p>
        </Link>
        <h2 className="font-bold text-[24px]">Личный кабинет</h2>
        ...
        >
          <span className="text-[#757575] font-[16px] transition-all duration-200 hover:text-red-500">Выйти</span>
        </button>
      </aside>
      ...
      <section className="flex-1 mt-[100px] ml-[33px]">
        {activeTab === 'data' && (
          <div className="animate-in fade-in">
            <h3 className="text-[18px] font-bold">Мои данные</h3>
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
              <div className="p-4 bg-white border border-gray-100 rounded-2xl shadow-sm">
                <p className="text-xs text-gray-400 uppercase font-bold mb-1">Имя и Фамилия</p>
                <p className="text-gray-800 font-medium">{user.first_name} {user.last_name}</p>
              </div>
              <div className="p-4 bg-white border border-gray-100 rounded-2xl shadow-sm">
                <p className="text-xs text-gray-400 uppercase font-bold mb-1">Телефон</p>
                <p className="text-gray-800 font-medium">{user.phone || 'Не указан'}</p>
              </div>
              <div className="p-4 bg-white border border-gray-100 rounded-2xl shadow-sm">
              ...
              <div className="p-4 bg-white border border-gray-100 rounded-2xl shadow-sm">
                <p className="text-xs text-gray-400 uppercase font-bold mb-1">Роль аккаунта</p>
        ...
        {activeTab === 'orders' && (
          <div className="animate-in fade-in">
            <h3 className="text-[18px] font-bold">Мои заказы</h3>
            <div className="flex items-center min-h-[209px]  gap-[50px] pt-[100px] ml-[190px]">
              ...
              <div className="flex flex-col gap-[20px] w-[300px]">
                <h4 className="font-bold text-[32px]">Пока ничего</h4>
                <p className="text-[20px]">Заказы появятся после первой покупки</p>
                <Link href="/" >
        ...
        {activeTab === 'addresses' && (
          <div className="animate-in fade-in">
            <div className="flex justify-between">
              <h3 className="text-[18px] font-bold">Мои адреса</h3>
              <button className="flex gap-[15px]">
                <svg width="24" height="24" viewBox="0 0 24 24" fill="none" xmlns="http://www.w3.org/2000/svg">
                  <path d="M12.75 4C12.75 3.58579 12.4142 3.25 12 3.25C11.5858 3.25 11.25 3.58579 11.25 4L11.25 11.25H4C3.58579 11.25 3.25 11.5858 3.25 12C3.25 12.4142 3.58579 12.75 4 12.75H11.25V20C11.25 20.4142 11.5858 20.75 12 20.75C12.4142 20.75 12.75 20.4142 12.75 20V12.75H20C20.4142 12.75 20.75 12.4142 20.75 12C20.75 11.5858 20.4142 11.25 20 11.25H12.75L12.75 4Z" fill="#1565C0"/>
                </svg>
...
import { useRouter } from 'next/navigation';
import { useAuthStore } from '@/store/useAuthStore';
import { LogOut, User, Package, MapPin, CreditCard, ChevronRight, Trash2 } from 'lucide-react';
import Link from 'next/link';
...
export default function ProfilePage() {
  ...
  const [editId, setEditId] = useState<number | null>(null); // ID редактируемого адреса
  ...
  const handleEditClick = (addr: any) => {
    // Проверяем, что addr.address существует, иначе берем пустую строку
    const fullString = addr.address || '';
    const parts = fullString.split(', ');
    const street = parts[0] || '';
    
    // Безопасный поиск значения
    const findValue = (key: string) => {
      if (!parts || parts.length <= 1) return ''; // Если доп. данных нет, возвращаем пусто
  ...
  const handleSaveAddress = async () => {
    ...
    try {
      const res = await fetch(url, {
        ...
        body: JSON.stringify({ 
          address: fullAddress, 
          is_default: editId ? undefined : addresses.length === 0,
  ...
  const handleDelete = async (id: number) => {
    try {
      await fetch(`http://localhost:3000/profile/addresses/${id}`, {
        method: 'DELETE',
        headers: { 'Authorization': `Bearer ${token}` }
      });
      fetchAddresses();
  ...
  // Твои оригинальные MenuItems
  const menuItems = [
    { id: 'data', label: 'Мои данные', icon: <svg width="24" height="24" viewBox="0 0 24 24" fill="none" xmlns="http://www.w3.org/2000/svg"><circle cx="12" cy="6" r="4" stroke="currentColor" strokeWidth="1.5"/><ellipse cx="12" cy="17" rx="7" ry="4" stroke="currentColor" strokeWidth="1.5"/></svg> },
    { id: 'orders', label: 'Мои заказы', icon: <svg width="24" height="24" viewBox="0 0 24 24" fill="none" xmlns="http://www.w3.org/2000/svg"><path d="M3.79375 12.0291C4.33092 9.34329 4.5995 8.00036 5.48697 7.13543C5.651 6.97557 5.82845 6.8301 6.01737 6.70061C7.03956 6 8.40907 6 11.1481 6H12.851C15.5901 6 16.9596 6 17.9818 6.70061C18.1707 6.8301 18.3482 6.97557 18.5122 7.13543C19.3996 8.00036 19.6682 9.34329 20.2054 12.0291C20.9766 15.8851 21.3622 17.8131 20.4745 19.1793C20.3138 19.4267 20.1262 19.6555 19.9152 19.8616C18.7496 21 16.7834 21 12.851 21H11.1481C7.21574 21 5.24955 21 4.08398 19.8616C3.87293 19.6555 3.68534 19.4267 3.52461 19.1793C2.63695 17.8131 3.02255 15.8851 3.79375 12.0291Z" stroke="currentColor" strokeWidth="1.5"/><circle cx="15" cy="9" r="1" fill="currentColor"/><circle cx="9" cy="9" r="1" fill="currentColor"/><path d="M9 6V5C9 3.34315 10.3431 2 12 2C13.6569 2 15 3.34315 15 5V6" stroke="currentColor" strokeWidth="1.5" strokeLinecap="round"/></svg> },
    { id: 'payment', label: 'Способы оплаты', icon: <svg width="24" height="24" viewBox="0 0 24 24" fill="none" xmlns="http://www.w3.org/2000/svg"><path d="M2 12C2 8.22876 2 6.34315 3.17157 5.17157C4.34315 4 6.22876 4 10 4H14C17.7712 4 19.6569 4 20.8284 5.17157C22 6.34315 22 8.22876 22 12C22 15.7712 22 17.6569 20.8284 18.8284C19.6569 20 17.7712 20 14 20H10C6.22876 20 4.34315 20 3.17157 18.8284C2 17.6569 2 15.7712 2 12Z" stroke="currentColor" strokeWidth="1.5"/><path d="M10 16.5H6" stroke="currentColor" strokeWidth="1.5" strokeLinecap="round"/><path d="M8 13.5H6" stroke="currentColor" strokeWidth="1.5" strokeLinecap="round"/><path d="M2 10L22 10" stroke="currentColor" strokeWidth="1.5" strokeLinecap="round"/><path d="M14 15C14 14.0572 14 13.5858 14.2929 13.2929C14.5858 13 15.0572 13 16 13C16.9428 13 17.4142 13 17.7071 13.2929C18 13.5858 18 14.0572 18 15C18 15.9428 18 16.4142 17.7071 16.7071C17.4142 17 16.9428 17 16 17C15.0572 17 14.5858 17 14.2929 16.7071C14 16.4142 14 15.9428 14 15Z" stroke="currentColor" strokeWidth="1.5"/></svg> },
    { id: 'addresses', label: 'Мои адреса', icon: <svg width="24" height="24" viewBox="0 0 24 24" fill="none" xmlns="http://www.w3.org/2000/svg"><path d="M12 22C7.28595 22 4.92893 22 3.46447 20.5355C2 19.0711 2 16.714 2 12C2 7.28595 2 4.92893 3.46447 3.46447C4.92893 2 7.28595 2 12 2C16.714 2 19.0711 2 20.5355 3.46447C22 4.92893 22 7.28595 22 12C22 16.714 22 19.0711 20.5355 20.5355C19.0711 22 16.714 22 12 22Z" stroke="currentColor" strokeWidth="1.5"/><path d="M13.4227 17.3618L16.9348 8.19598C17.2164 7.46107 16.5389 6.78361 15.804 7.06521L6.63824 10.5773C5.80779 10.8955 5.78079 12.06 6.5981 12.3083L10.0751 13.3648C10.3455 13.447 10.553 13.6545 10.6352 13.9249L11.6917 17.4019C11.94 18.2192 13.1045 18.1922 13.4227 17.3618Z" stroke="currentColor" strokeWidth="1.5"/></svg> },
  ];
  ...
  return (
    <div className="w-[1340px] mx-auto py-[20px] flex">
      ...
      <aside>
        <Link href="/" className="flex gap-[10px] items-center my-[20px]">
          <svg width="20" height="20" viewBox="0 0 20 20" fill="none" xmlns="http://www.w3.org/2000/svg"><path d="M12.5 15.8334L7.5 10L12.5 4.16671" stroke="#212121" strokeWidth="1.5" strokeLinecap="round" strokeLinejoin="round"/></svg>
          <p className="text-[16px]">Вернуться</p>
        </Link>
        <h2 className="font-bold text-[24px]">Личный кабинет</h2>
        <nav className="flex flex-col gap-[15px] mt-[20px]">
          {menuItems.map((item) => (
            <button key={item.id} onClick={() => setActiveTab(item.id)} className={`flex items-center justify-between transition-all duration-200 ${activeTab === item.id ? ' text-[#1565C0]' : 'hover:bg-gray-50 text-[#757575]'}`}>
              <div className="flex items-center gap-3 transition-all duration-200 hover:translate-x-1">
                {item.icon}
                <span className="text-[16px]">{item.label}</span>
              </div>
        ...
        <button onClick={handleLogout} className="mt-[50px] hover:opacity-80">
          <span className="text-[#757575] font-[16px] transition-all duration-200 hover:text-red-500">Выйти</span>
        </button>
      </aside>
...
{deleteId !== null && (
  <div className="fixed inset-0 bg-black/50 z-[110] flex items-center justify-center p-4 animate-in fade-in duration-200">
    <div className="bg-white rounded-[30px] p-[40px] w-full max-w-[400px] text-center shadow-2xl">
      <h3 className="text-[20px] font-bold mb-2">Удалить адрес?</h3>
      <p className="text-gray-500 mb-8">Это действие нельзя будет отменить</p>
      <div className="flex gap-3">
        <button onClick={() => setDeleteId(null)} className="flex-1 py-3 border border-gray-200 rounded-xl font-bold hover:bg-gray-50 transition-colors">Отмена</button>
        <button onClick={confirmDelete} className="flex-1 py-3 bg-red-500 text-white rounded-xl font-bold hover:bg-red-600 transition-colors">Удалить</button>
      </div>
    </div>
  </div>
)}

{/* МОДАЛЬНОЕ ОКНО АДРЕСА (ДОБАВЛЕНИЕ / РЕДАКТИРОВАНИЕ) }
{isAdding && (
  <div className="fixed inset-0 bg-black/50 z-[100] flex items-center justify-center p-4 animate-in fade-in duration-200">
    <div className="bg-white rounded-[30px] p-[40px] w-full max-w-[500px] animate-in zoom-in-95 duration-200 shadow-2xl">
      <h3 className="text-[20px] font-bold mb-6">
        {editId ? 'Редактировать адрес' : 'Новый адрес доставки'}
      </h3>
      <div className="flex flex-col gap-4">
        ...
        <div className="grid grid-cols-3 gap-2">
          <input placeholder="Подъезд" className="p-4 border border-gray-100 rounded-xl outline-none focus:border-[#1565C0] bg-gray-50/30" value={addressData.entrance} onChange={e => setAddressData({...addressData, entrance: e.target.value})}/>
          <input placeholder="Этаж" className="p-4 border border-gray-100 rounded-xl outline-none focus:border-[#1565C0] bg-gray-50/30" value={addressData.floor} onChange={e => setAddressData({...addressData, floor: e.target.value})}/>
          <input placeholder="Кв/офис" className="p-4 border border-gray-100 rounded-xl outline-none focus:border-[#1565C0] bg-gray-50/30" value={addressData.apartment} onChange={e => setAddressData({...addressData, apartment: e.target.value})}/>
        </div>
        <textarea 
        ...
        <div className="flex gap-3 mt-4">
          <button 
            onClick={() => { setIsAdding(false); setEditId(null); setAddressData({street:'', floor:'', entrance:'', apartment:'', comment:''}); }} 
          ...
          <button 
            ...
            className="flex-1 py-4 bg-[#1565C0] text-white rounded-xl font-bold shadow-lg shadow-blue-200"
          >
...
{activeTab === 'addresses' && (
  <div className="animate-in fade-in">
    <div className="flex justify-between items-center mb-6">
      <h3 className="text-[18px] font-bold">Мои адреса</h3>
      <button onClick={() => { setEditId(null); setIsAdding(true); }} className="flex gap-[12px] items-center group">
        <div className="w-8 h-8 bg-blue-50 rounded-full flex items-center justify-center group-hover:bg-blue-100 transition-colors">
            <svg width="20" height="20" viewBox="0 0 24 24" fill="none" xmlns="http://www.w3.org/2000/svg"><path d="M12 5V19M5 12H19" stroke="#1565C0" strokeWidth="2" strokeLinecap="round"/></svg>
        </div>
        <p className="text-[#1565C0] text-[16px] font-bold">Добавить адрес</p>
      </button>
    </div>
    ...
    <div className="space-y-3">
      {addresses.length > 0 ? (
        addresses.map((addr) => {
          ...
          return (
            <div key={addr.id} className="p-5 border-2 border-gray-50 rounded-[25px] flex justify-between items-center bg-white transition-all">
              <div className="flex items-center gap-4 flex-1">
                ...
                <div className="flex flex-col">
                  <p className={`font-bold text-[16px] leading-tight ${addr.is_default ? 'text-[#1565C0]' : 'text-[#212121]'}`}>
                    {streetPart}
                  </p>
                  {detailsPart && <p className="text-[14px] text-gray-400 mt-1">{detailsPart}</p>}
                </div>
              </div>
              ...
              <div className="flex items-center gap-2">
                ...
                <button 
                  ...
                  className="text-gray-300 hover:text-[#1565C0] transition-colors p-2"
                >
                  <svg width="20" height="20" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round"><path d="M11 4H4a2 2 0 0 0-2 2v14a2 2 0 0 0 2 2h14a2 2 0 0 0 2-2v-7"/><path d="M18.5 2.5a2.121 2.121 0 0 1 3 3L12 15l-4 1 1-4 9.5-9.5z"/></svg>
                </button>

                <button onClick={() => setDeleteId(addr.id)} className="text-gray-300 hover:text-red-500 p-2">
                  <Trash2 size={20} />
                </button>
              </div>
            </div>
        ...
        <div className="py-10 border-2 border-dashed border-gray-200 rounded-[25px] text-center text-gray-400">
          Адреса не добавлены
        </div>
      )}
    </div>
        ...
        {activeTab === 'data' && (
          <div className="animate-in fade-in">
            <h3 className="text-[18px] font-bold">Мои данные</h3>
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-4 mt-4 text-[#212121]">
              <div className="p-4 border border-gray-50 rounded-2xl"><p className="text-xs text-gray-400 mb-1">Имя и Фамилия</p><p className="font-medium">{user.first_name} {user.last_name}</p></div>
              <div className="p-4 border border-gray-50 rounded-2xl"><p className="text-xs text-gray-400 mb-1">Телефон</p><p className="font-medium">{user.phone || 'Не указан'}</p></div>
              <div className="p-4 border border-gray-50 rounded-2xl"><p className="text-xs text-gray-400 mb-1">Email</p><p className="font-medium">{user.email}</p></div>
              <div className="p-4 border border-gray-50 rounded-2xl"><p className="text-xs text-gray-400 mb-1">Роль аккаунта</p><p className="font-medium capitalize">{user.role}</p></div>
            </div>
          </div>
        )}
        {activeTab === 'orders' && <div className="animate-in fade-in"><h3 className="text-[18px] font-bold">Мои заказы</h3><div className="mt-4 py-10 border-2 border-dashed border-gray-200 rounded-[25px] text-center text-gray-400">Заказов пока нет</div></div>}
        {activeTab === 'payment' && <div className="animate-in fade-in"><h3 className="text-[18px] font-bold">Способы оплаты</h3><div className="mt-4 py-10 border-2 border-dashed border-gray-200 rounded-[25px] text-center text-gray-400">Карты не привязаны</div></div>}
...
export default function ProfilePage() {
  ...
  // 1. Создаем состояние для заказов
  const [orders, setOrders] = useState([]);
  const [isLoadingOrders, setIsLoadingOrders] = useState(false);

  // 2. Загружаем заказы при монтировании или смене вкладки
  useEffect(() => {
  ...
  useEffect(() => {
    const handleUrlChange = () => {
      ...
      // 1. Если в ссылке ?tab=cards
      if (tab === 'cards') {
        setActiveTab('payment');
      } 
      // 2. Если в ссылке #addresses
      else if (hash === '#addresses') {
        setActiveTab('addresses');
      } 
      // 3. Если переходим просто на профиль (без хеша или с #data)
    ...
    // Слушаем изменение URL (хеша), чтобы ссылки из шапки работали мгновенно
    window.addEventListener('hashchange', handleUrlChange);
    return () => window.removeEventListener('hashchange', handleUrlChange);
  }, [searchParams]);

  useEffect(() => {
    // Проверяем наличие хеша в URL при загрузке страницы
    if (window.location.hash === '#addresses') {
      setActiveTab('addresses');
      
      // Опционально: плавно скроллим к разделу, если страница длинная
  ...
  const handleSaveAddress = async () => {
    ...
    try {
      ...
      if (res.ok) {
        // Если мы редактировали текущий дефолтный адрес ИЛИ добавили самый первый адрес
        if (editId) {
          const editingAddr = addresses.find(a => a.id === editId);
          if (editingAddr?.is_default) {
            setUserAddress(addressData.street); // Обновляем шапку
          }
        } else if (addresses.length === 0) {
          setUserAddress(addressData.street); // Обновляем шапку, если это первый адрес
        }
  ...
  const confirmDelete = async () => {
    ...
    // 1. Находим адрес, который хотим удалить, чтобы проверить, был ли он активным
    ...
    try {
      ...
      if (res.ok) {
        // 2. Получаем свежий список адресов после удаления
        const response = await fetch('http://localhost:3000/profile/addresses', {
          headers: { 'Authorization': `Bearer ${token}` }
        });
        const updatedAddresses = await response.json();
        
        // Сортируем, чтобы порядок оставался стабильным
        const sorted = updatedAddresses.sort((a: any, b: any) => a.id - b.id);
        setAddresses(sorted);

        // 3. Логика переключения активного адреса
        if (sorted.length === 0) {
          // Если адресов не осталось совсем — сбрасываем шапку
          setUserAddress('Укажите адрес');
        } else if (wasDefault) {
          // Если удалили активный, но есть другие — делаем первый из списка основным
          const nextAddress = sorted[0];
          await handleSetDefault(nextAddress.id); 
          // handleSetDefault сам вызовет setUserAddress и обновит шапку
        }
  ...
  try {
    ...
    if (res.ok) {
      // Находим адрес в локальном списке, который мы только что сделали основным
      const selectedAddr = addresses.find(a => a.id === id);
      if (selectedAddr) {
        // Обновляем адрес в шапке (берем только улицу и дом)
  ...
  const handleLogout = () => {
    logout();
    setIsLogoutModalOpen(false); // Закрываем модалку
    router.push('/login');
  };
        ...
        {isLogoutModalOpen && (
          <div className="fixed inset-0 bg-black/50 flex items-center justify-center z-[100] backdrop-blur-sm">
            <div className="bg-white p-[25px] rounded-[12px] w-[430px] shadow-2xl scale-in-center">
              <h2 className="text-2xl font-bold mb-4 text-[#212121] text-center">Выход из аккаунта</h2>
              <p className="text-[#757575] mb-8 text-center">
                Вы уверены, что хотите выйти? Чтобы совершать покупки, вам придется войти снова.
              </p>
        ...
        {deleteId !== null && (
          <div className="fixed inset-0 bg-black/50 backdrop-blur-sm flex items-center justify-center z-50 animate-in fade-in duration-200">
            <div className="bg-white p-[24px] rounded-[16px] max-w-[400px] w-full shadow-2xl scale-in-center">
              ...
              <h3 className="text-[20px] font-bold mt-[16px] text-[#181D27]">Удалить адрес</h3>
        ...
        {isAdding && (
          <div className="fixed inset-0 bg-black/50 z-[100] flex items-center justify-center p-4 animate-in fade-in duration-200">
            <div className="bg-white rounded-[16px] p-[25px] w-full max-w-[700px] animate-in zoom-in-95 duration-200 shadow-2xl">
              <h3 className="text-[20px] font-bold mb-6">
                {editId ? 'Редактировать адрес' : 'Добавление адреса'}
              </h3>
              <div className="flex flex-col gap-0">
                <p className="ml-[5px]">Адрес</p>
                <input 
                ...
                <div className="grid grid-cols-3 gap-2 mt-[20px]">
                  <div>
                    <p className="ml-[5px]">Этаж</p>
                    <input maxLength={10} placeholder="Этаж" className="w-[200px] p-4 border border-gray-100 rounded-xl outline-none focus:border-[#1565C0] bg-gray-50/30" value={addressData.entrance} onChange={e => setAddressData({...addressData, entrance: e.target.value})}/>
                  </div>
                  <div>
                    <p className="ml-[5px]">Подъезд</p>
                    <input maxLength={10} placeholder="Подъезд" className="w-[200px] p-4 border border-gray-100 rounded-xl outline-none focus:border-[#1565C0] bg-gray-50/30" value={addressData.floor} onChange={e => setAddressData({...addressData, floor: e.target.value})}/>
                  </div>
                  <div>
                    <p className="ml-[5px]">Квартира</p>
                    <input maxLength={10} placeholder="Квартира" className="w-[200px] p-4 border border-gray-100 rounded-xl outline-none focus:border-[#1565C0] bg-gray-50/30" value={addressData.apartment} onChange={e => setAddressData({...addressData, apartment: e.target.value})}/>
                  </div>
                </div>
                <p className="mt-[20px] ml-[5px]">Комментарий курьеру</p>
                <textarea 
                  placeholder="Оставьте ваш комментарий для курьера" 
                  className="p-4 border border-gray-100 rounded-xl outline-none h-[100px] resize-none focus:border-[#1565C0] bg-gray-50/30"
                  value={addressData.comment}
                  onChange={e => setAddressData({...addressData, comment: e.target.value})}
                />
