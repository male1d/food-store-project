...
export default async function ProductPage({ 
  ...
  return (
    <div className="w-[1340px] mx-auto min-h-[600px]">
      <Link locale={locale} href="/" className="flex gap-[10px] items-center my-[20px]">
      ...
      <div className="flex mb-[60px] text-[#757575] text-[13px] gap-[10px]">
        ...
        <p>{product.category?.name || `${t("prod")}`}</p>
      </div>
      <div className="flex gap-[50px]">
        <div className="w-[486px] bg-[#F8F9FA] rounded-[15px] overflow-hidden flex items-center justify-center border border-gray-200">
          {product.image_url ? (
            ...
            <div className="text-gray-300 font-bold text-xl uppercase tracking-widest">{t("noPhoto")}</div>
          )}
        </div>
        ...
        <div className="w-fill flex flex-col gap-[20px] max-w-[864px]">
          <div>
            ...
            <p className="text-[20px] font-bold">{product.unit || `${t("pieces")}`}</p>
          ...
          <h3 className="text-[18px] font-bold">{t("value")}</h3>
          <div>
            <h3 className="text-[18px] font-bold">{t("des")}</h3>
            <p className="leading-relaxed text-[15px]">
              {product.description || `${t("description")}`}
            </p>
          </div>
          <h3 className="text-[18px] font-bold">{t("composition")}</h3>
          <h3 className="text-[18px] font-bold">{t("life")}</h3>
        </div>
      </div>
    </div>
  );
