...
export default async function Home({
  ...
  const activeCategoryName = categories.find(c => String(c.id) === currentCategory)?.name || t("allProduct");
  ...
  return (
    <div className="px-[20px] flex gap-[10px]">
      <section className="w-[271px] flex flex-col pt-[20px] gap-[10px]">
        <Link href="/" locale={locale} className={!currentCategory ? "flex items-center gap-[10px]" : "flex items-center gap-[10px]"}>
        ...
        <div>
            ...
            <p className="text-gray-400 text-sm">{t('products_count', { count: totalFilteredCount })}</p>
        </div>
        <h3 className="text-[18px] font-bold mt-[20px]">{t("filter")}</h3>
        <h3 className="text-[16px]">{t("price")}</h3>
        ...
        <div className="grid grid-cols-4 gap-y-[20px] gap-x-auto pt-[20px]">
          {currentProducts.length > 0 ? currentProducts.map((product) => (
            <div key={product.id} className="w-max-[260px] p-[10px] h-[412px] hover:shadow-lg transition-shadow border border-transparent hover:border-gray-100 rounded-3xl">
              <Link locale={locale} href={`/product/${product.id}`} className="flex flex-col gap-[5px] cursor-pointer group">
                <div className="h-[233px] bg-gray-100 rounded-2xl flex items-center justify-center text-gray-400 overflow-hidden">
                  {product.image_url ? (
                    ...
                    <span className="text-xs text-center px-4 italic">{t("willAppear")}</span>
                  )}
                </div>
            ...
            <div className="col-span-4 text-center py-20 text-gray-400">{t("productsF")}</div>
          )}
        </div>
